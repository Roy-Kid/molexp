...
- **Public surface is deliberately small**: `molexp.harness.__all__` = 19 symbols (`Mode` / `Stage` / `StageRunner` / `HarnessRunContext` / `PlanMode` / `ModeResult` / `ArtifactStore` / `FileArtifactStore` / `Executor` / `LocalExecutor` / `DryRunExecutor` / `ApprovalGate` / `AgentGateway` + the doc-cited `RouterBackedAgentGateway` / `CapabilityRegistry` / `StageExecutionError` / `SQLiteEventLog` / `SQLiteArtifactLineageStore` / `replay_metadata`), locked by `tests/test_harness/test_public_surface.py`. Stages / schemas / validators import from their subpackages (`harness.stages.*` / `harness.schemas.*`), never from the top level. Plan-owned schema names carry the `Plan` prefix to avoid cross-layer collisions: `PlanTaskIR` / `PlanWorkflowIR` / `PlanValidationReport` / `PlanArtifactRef`. `PlanMode.step_groups()` (`PlanStep`) is the single source of the nine-step grouping — the CLI banner shows steps, internal stage names only under `molexp plan -v`. The plan CLI **preflights before any disk write** (model → agent-stack import → credential resolution, via `services.plan_runtime.preflight_plan_router`), so a failed `molexp plan` leaves zero residue.

**`molexp.services`** — application-service layer between CLI/server and the four domain layers.
- Owns: `plan_runtime` (PlanMode driving + preflight + `materialize` for plan records), `curate_runtime`, `operator_config` (the one `agent.model` loader shared by CLI and server), `agent_task_store`. This is where "Python 操作 = UI 操作" is enforced: CLI commands and server routes both call services, never each other.
