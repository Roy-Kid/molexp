...
- **Allowed imports**: stdlib / pydantic only (no pyyaml needed). **MUST NOT** import `workspace` or any upstream layer (`workflow` / `agent` / `harness` / `server` / `cli` / `plugins` / `sweep`) — enforced by `tests/test_knowledge/test_import_guard.py`, an **AST source scan** (not a `sys.modules` probe: `molexp/__init__.py` eagerly loads `workspace`, so a runtime probe is unsatisfiable).

**`molexp.workspace`** — bottom; pure storage.
- Owns: `Folder` base + the `Workspace/Project/Experiment/Run` subclasses, typed exceptions (`*NotFoundError` / `*ExistsError`), atomic JSON I/O (`atomic_write_json`), the `Asset` family + per-scope `AssetManifest` (`assets.json`) + the `assets.scan` manifest-scanning query layer (`scan_assets` / `get_asset` / `find_by_content_hash`), `Params` / `ParamSpace` / `GridSpace` / `UniformSpace`, `ComputeTarget`, `RunContext`, the OKF Concept surface (`Note` / `ReferenceConcept` + `ReferenceMeta`, the `Bundle` façade, `ZoteroItem` / `read_zotero_items`), and one singleton folder accessed as a lowercase property: `ws.cache` (`CacheFolder` → `as_cache_store()` adapter). **There is no derived SQLite asset index** — asset queries scan the authoritative per-scope `assets.json` manifests (+ directly-registered `assets/<id>/asset.json` records); the former `AssetCatalog` / `catalog/index.sqlite` was removed (`workspace-git-projection-01`).
...
- Owns: artifact lineage + audit + execution machinery (`HarnessRunContext`, `Stage`, `StageRunner`, `ArtifactStore` + `FileArtifactStore`, `EventLog` + `SQLiteEventLog`, `ArtifactLineageStore` + `SQLiteArtifactLineageStore`, `CapabilityRegistry`, executors, validators, `ApprovalGate`, `generate_audit_report` + replay). Run-level provenance (params, config hash, code/script identity) is owned by **workspace** (`RunMetadata` + per-scope `AssetManifest`); harness lineage covers agent-pipeline artifacts only and stamps each edge with stage + workspace `run_id`. One canonical mode pipeline ships under `harness/modes/`, each stage under `harness/stages/`. `PlanMode(approver=None, executor=None, *, execute=False, compute_target=None)` runs **nine visible steps** (the `--execute` tail is opt-in): (1) **Draft proposal** `SaveUserPlan → GenerateExperimentReport`; (2) **Draft spec** `GenerateExperimentSpec → ValidateExperimentSpec` (concrete `experiment_spec`, JSON; rendered at the CLI/server boundary as one whole YAML that folds the step-4 workflow IR in as a trailing `workflow_spec:` section — not split into per-field blocks); (3) **Resolve capabilities** `ResolveCapabilities` (molmcp grounds the *full* toolchain onto `ctx.capability_registry`; an LLM — the `capability_selector` agent — then picks the **minimal** subset this experiment needs, and the stage emits only those into the `capability_catalog` artifact `BindMolcraftsTasks` consumes; the registry stays full, so `ValidateBoundWorkflow` still validates any bound id); (4) **Workflow IR** `ExtractWorkflowIR` (reads `experiment_spec`) `→ ValidateWorkflowIR`; (5) **Tasks + per-task tests** `BindMolcraftsTasks → ValidateBoundWorkflow → GenerateWorkflowSource → ValidateWorkflowSource/ReviewPlan → GenerateTestSpec → ValidateTestSpec → GenerateTestCode → ValidateTestSource`; (6) **Input set** `GenerateInputSet → ValidateInputSet` (`input_set` sweep, bridges to workspace `GridSpace`/`UniformSpace`); (7) **Compile / dry-run** `MaterializeExecution → ExecuteTests → CompileWorkflow` (`run_workflow.py --compile-only` — compiles + runs the per-task tests, **no** real science; produces an `execution_result` tagged `metadata.mode="compile"`); (8) **Review** `ApprovalGate(approve_plan)` (the single human/auto gate; `PlanMode(approver=…)` wires it); (9) **Execution report** `GenerateExecutionReport` (descriptive `execution_report` — machine/scheduler/account/resource-policy/env from the bound workflow + an injected `ComputeTarget`; **never submits**). The default flow ends at the execution report; `execute=True` appends the real-execution tail — `ExecuteWorkflow → GenerateFinalReport → ApprovalGate(approve_execution) → GenerateAuditReport` (the folded-in former RunMode back half, now retired) — where pytest and the materialized `run_workflow.py` driver run as **executor subprocesses** (default `LocalExecutor`; inject `DryRunExecutor` to skip the step-7 subprocesses), so the `molexp.workflow` engine never loads in the harness process. The `harness/gateways/` subpackage holds the `AgentGateway` Protocol + `StubAgentGateway` (test stub) + `RouterBackedAgentGateway` (production impl driven by `agent.router.Router`); the plan LLM-agent registry (schema + output-kind + system-prompt per `agent_name`) is the single shared `harness.gateways.plan_agents` (consumed identically by CLI + server). **Production entry point: `molexp plan [--execute]` (`cli/plan_cmd.py`)** — drives the single PlanMode against a content-addressed Run (same draft → same Run → the stage ledger resumes/skips completed stages); artifacts land under `run_dir/artifacts`, events + artifact lineage in `run_dir/harness.sqlite`.
