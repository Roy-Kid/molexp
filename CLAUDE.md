...
- **Run status × verb selection (canonical).** A `Run` is `pending → running → succeeded | failed | cancelled`; a live `running` whose owner PID is dead is reaped to `failed` *before* any verb decides. **Plain run** (no verb) only starts work that has not run: create missing + run `pending`; it leaves `succeeded` / `running` / `failed` / `cancelled` alone (retrying a failure is an explicit verb, never implicit). **resume** and **rerun** retry an *existing, profile-matching, non-succeeded* run — they differ only in how they treat a finished non-succeeded execution (resume reopens+seeds it; rerun starts a fresh `exec-{run_id}-N`); on a `pending` run both just run its first execution. **All three skip `succeeded` and a *live* `running` run** — a Run carries one ownership stamp + one status, so a second concurrent execution is never started; to retry a stuck-but-live run, `kill` it first (→ `cancelled`/`failed`) then `resume`/`rerun`.
