...
- Owns: the public surface (`AgentRunner`, `AgentLoop`, `AgentRunResult`, `AgentRuntime`, `AgentSession`, approval primitives). `AgentRuntime` is the frozen-dataclass bundle (`session` + `router` + `execution_env` + `hooks`) a loop receives at run time. Flat module list — `loop.py` / `runtime.py` / `session.py` / `session_storage.py` / `session_entry.py` / `events.py` (`AgentEvent` discriminated union + `AsyncIteratorEventSink`) / `compaction.py` / `execution_env.py` / `hooks.py` / `router.py` (Protocol) / `runner.py` / `review.py` / `folders.py`. Two concrete loops under `loops/`: `ChatLoop` (one round-trip) and `InteractiveLoop` (emergent tool loop driving `Router.stream_agentic`). Private `_pydanticai/` is the **sole** sanctioned `import pydantic_ai` site (`router.py`, `mcp.py`, `messages_codec.py`, …).
