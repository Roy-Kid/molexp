...
// The plan→experiment provenance edge (vision-loop-10): an AI-generated
// experiment carries `planRunId` — the plan run that produced its workflow.
// A plan IS a run, so the ref resolves through the snapshot's runs.
const planGroup = (
  experiment: ExperimentSummary | undefined,
  snapshot: WorkspaceSnapshot,
): RelationGroup => {
  const planRunId = experiment?.planRunId;
  const planRun = planRunId ? snapshot.runs.find((r) => r.id === planRunId) : undefined;
  return group("plan", "Generated by plan", planRun ? [runRef(planRun)] : []);
};
