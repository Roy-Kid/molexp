...
describe("plan provenance group (vision-loop-10)", () => {
  const withPlanRun = (planRunId: string | null): WorkspaceSnapshot => ({
    ...snapshot,
    experiments: [{ ...snapshot.experiments[0], planRunId }],
  });

  it("links an experiment to the plan run that generated it", () => {
    const rel = resolveRelations({ kind: "experiment", id: "e1" }, withPlanRun("r2"));
    expect(groupRefs(rel, "plan")).toEqual(["r2"]);
    expect(rel.find((g) => g.relation === "plan")?.label).toBe("Generated by plan");
  });

  it("derives a run's plan group via its parent experiment", () => {
    const rel = resolveRelations({ kind: "run", id: "r1" }, withPlanRun("r2"));
    expect(groupRefs(rel, "plan")).toEqual(["r2"]);
  });
  ...
  it("omits the plan group when planRunId does not resolve in the snapshot", () => {
    const rel = resolveRelations({ kind: "experiment", id: "e1" }, withPlanRun("ghost"));
    expect(rel.find((g) => g.relation === "plan")).toBeUndefined();
  });
});
