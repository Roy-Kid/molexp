...
describe("workbench motion contract", () => {
  it("centralizes durations, easing, reduced motion, and progress cadence", () => {
    ...
    expect(css).toContain("--default-transition-duration: var(--motion-base)");
    expect(css).toContain("--default-transition-timing-function: var(--motion-ease)");
    ...
    expect(css).toContain(
      "*:not(.mol-motion-progress-spin):not(.mol-motion-progress-pulse):not(\n        .mol-progress-spinner\n      )",
    );
