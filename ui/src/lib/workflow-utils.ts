...
import type { TaskNodeJson, EdgeJson, TaskGraphJson } from '@/types/task_graph_ir';
 ...
 */
export const analyzePaths = (nodes: Node[], edges: Edge[], targets: string[]): Record<string, string[]> => {
    ...
    targets.forEach(targetId => {
        ...
        while (queue.length > 0) {
            ...
            (edgeDeps[curr] || []).forEach(edgeId => add(edgeId, targetId));

            // Continue to dependencies
            (deps[curr] || []).forEach(dep => queue.push(dep));
        }
    });
