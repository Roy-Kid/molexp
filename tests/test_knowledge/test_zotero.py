"""Tests for read-only Zotero import → Reference concepts (okf-07-02).

A minimal ``zotero.sqlite`` is built in-place (the real schema subset the
reader touches), then imported. PDFs are pointed at via ``pdf_path`` — no bytes
...
from molexp.knowledge import Library, Reference, ZoteroItem
...
# ── read_zotero_items (ac-001 / ac-002) ──────────────────────────────────────


def test_read_zotero_items_parses_fields(tmp_path: Path) -> None:
    from molexp.knowledge.zotero import read_zotero_items
...
# ── Library.import_zotero (ac-003 / ac-004 / ac-005) ─────────────────────────
...
def test_import_zotero_creates_reference_pointers(tmp_path: Path) -> None:
    ...
    refs = lib.import_zotero(db, now=FIXED)
    assert all(isinstance(r, Reference) for r in refs)
    by_key = {r.read_ref_meta().source_key: r for r in lib.references()}
...
def test_import_zotero_idempotent_on_source_key(tmp_path: Path) -> None:
    ...
    lib.import_zotero(db, now=FIXED)
    lib.import_zotero(db, now=FIXED)  # re-import
    assert len(lib.references()) == 2  # no duplicates
