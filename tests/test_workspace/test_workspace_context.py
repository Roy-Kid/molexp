...
RED-first: ``molexp.workspace.workspace_context`` does not exist yet.
...
def test_empty_workspace_is_explicit(tmp_path: Path) -> None:
...
def test_focus_passthrough_writes_nothing(tmp_path: Path) -> None:
    ws = _ws(tmp_path)
...
def test_assemble_is_pure(tmp_path: Path) -> None:
    ws = _ws(tmp_path)
    ws.add_project("p")
    a = assemble_workspace_context(ws)
    before = _tree(ws.resolve())
    b = assemble_workspace_context(ws)
    assert _tree(ws.resolve()) == before
    assert a == b
