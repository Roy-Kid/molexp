...
RED-first: ``molexp.workspace.knowledge_item`` does not exist yet.
...
def test_empty_sources_raises() -> None:
    """ac-001 — the invariant: a KnowledgeItem cannot be sourceless."""
...
def test_run_source_is_typed_edge_and_reachable(tmp_path: Path) -> None:
    ...
    assert os.path.normpath(str(run.resolve())) in {os.path.normpath(e) for e in item.out_edges()}


def test_artifact_source_meta_only(tmp_path: Path) -> None:
...
def test_concept_from_dir_reconstructs(tmp_path: Path) -> None:
