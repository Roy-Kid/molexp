...
def test_safety_floor_ignores_node_modules_without_gitignore(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
...
def test_nested_gitignore_applies_under_subdir(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    ...
    # Nested negation only un-ignores within that file's scope for paths
    # relative to data/ — keep.tmp under data/ was ignored by root *.tmp,
    # nested !keep.tmp re-includes it.
...
def test_root_never_ignored(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
    (root / ".gitignore").write_text("*\n")
