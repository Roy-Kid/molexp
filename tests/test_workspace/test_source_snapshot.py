...
class TestSnapshotSources:
    def test_captures_entrypoint_and_transitive_first_party_imports(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        src.mkdir()
    ...
    def test_excludes_nonlocal_imports_and_is_idempotent(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        src.mkdir()
