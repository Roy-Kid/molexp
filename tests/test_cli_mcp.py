...
class TestAdd:
    def test_stdio_default(self, cfg):
        result = runner.invoke(
            app, ["mcp", "add", "molmcp", "molmcp", "--config", str(cfg)]
        )
    ...
    def test_duplicate_without_force_fails(self, cfg):
        ...
        result = runner.invoke(
            app, ["mcp", "add", "n", "other", "--config", str(cfg)]
        )
    ...
    def test_duplicate_with_force_overwrites(self, cfg):
        runner.invoke(app, ["mcp", "add", "n", "cmd", "--config", str(cfg)])
        result = runner.invoke(
            app, ["mcp", "add", "n", "other", "-f", "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
...
class TestAddJson:
    def test_stores_arbitrary_shape(self, cfg):
        ...
        result = runner.invoke(
            app, ["mcp", "add-json", "molmcp", payload, "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
    ...
    def test_invalid_json(self, cfg):
        result = runner.invoke(
            app, ["mcp", "add-json", "x", "{not json", "--config", str(cfg)]
        )
        assert result.exit_code == 1
...
class TestScope:
    def test_user_scope_resolves_to_claude_json(self):
        from molexp.cli.mcp_cmd import _resolve_path

        path = _resolve_path("user", None)
    ...
    def test_project_scope_resolves_to_cwd_mcp_json(self, tmp_path, monkeypatch):
        ...
        path = _resolve_path("project", None)
    ...
    def test_config_override_wins(self, tmp_path):
        ...
        assert _resolve_path("user", explicit) == explicit.resolve()
        assert _resolve_path("project", explicit) == explicit.resolve()

    def test_unknown_scope_rejected(self):
        from molexp.cli.mcp_cmd import _resolve_path

        with pytest.raises(typer.BadParameter):
            _resolve_path("galaxy", None)
...
class TestImport:
    def test_skips_existing_without_force(self, cfg, tmp_path):
        ...
        runner.invoke(
            app, ["mcp", "add", "shared", "from-dst", "--config", str(cfg)]
        )
        result = runner.invoke(
            app, ["mcp", "import", str(src), "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
        servers = _read(cfg)["mcpServers"]
        assert servers["shared"]["command"] == "from-dst"  # kept
        assert servers["newone"]["command"] == "src-only"  # added
        assert "skipped:     shared" in result.output
    ...
    def test_force_overwrites(self, cfg, tmp_path):
        ...
        src.write_text(
            json.dumps(
                {"mcpServers": {"shared": {"type": "stdio", "command": "from-src"}}}
            )
        )
        runner.invoke(
            app, ["mcp", "add", "shared", "from-dst", "--config", str(cfg)]
        ...
        result = runner.invoke(
            app, ["mcp", "import", str(src), "-f", "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
...
class TestExport:
    def test_copies_to_destination(self, cfg, tmp_path):
        ...
        result = runner.invoke(
            app, ["mcp", "export", str(dst), "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
    ...
    def test_skips_existing_in_destination(self, cfg, tmp_path):
        ...
        dst = tmp_path / "dst.json"
        dst.write_text(
            json.dumps({"mcpServers": {"n": {"type": "stdio", "command": "from-dst"}}})
        )
        result = runner.invoke(
            app, ["mcp", "export", str(dst), "--config", str(cfg)]
        )
    ...
    def test_force_overwrites(self, cfg, tmp_path):
        ...
        result = runner.invoke(
            app, ["mcp", "export", str(dst), "-f", "--config", str(cfg)]
        )
        assert result.exit_code == 0, result.output
