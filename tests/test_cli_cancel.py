...
def _make_workspace(tmp_path, status="pending", job_id=None, scheduler_job_id=None):
    ...
    if job_id or scheduler_job_id:
        ...
        if job_id:
            executor_info["job_id"] = job_id
        if scheduler_job_id:
            executor_info["scheduler_job_id"] = scheduler_job_id
...
class TestRunsCancelMolqIntegration:
    ...
    def test_cancel_local_scheduler_skips_molq(self, tmp_path):
        ws_path, project, exp, run = _make_workspace(tmp_path, job_id="molq-uuid-local")
