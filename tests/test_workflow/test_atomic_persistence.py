...
def test_persistence_imports_workspace_atomic_write_json() -> None:
    """``RunStorePersistence`` reaches workspace's public atomic-write helper."""
...
def test_persistence_uses_atomic_write_json_for_workflow_json() -> None:
    """The body of ``_write_workflow`` calls ``atomic_write_json``."""
    text = PERSISTENCE_FILE.read_text()
    tree = ast.parse(text)
    target_func = next(
        (
            ...
            if isinstance(node, ast.FunctionDef) and node.name == "_write_workflow"
        ),
        None,
    )
    assert target_func is not None, "expected _write_workflow function"

    func_src = ast.get_source_segment(text, target_func) or ""
    assert "atomic_write_json" in func_src, (
        "_write_workflow must call atomic_write_json, not raw tmp.write_text"
    )


def test_persistence_drops_raw_tmp_write_pattern() -> None:
    """No more ``tmp.write_text(json.dumps(...))`` in ``_write_workflow``."""
    text = PERSISTENCE_FILE.read_text()
    tree = ast.parse(text)
    target_func = next(
        (
    ...
    assert "tmp.write_text" not in func_src, (
        "_write_workflow must not call tmp.write_text directly — use atomic_write_json"
    )
    assert "tmp.replace(" not in func_src, (
        "_write_workflow must not call tmp.replace directly — use atomic_write_json"
    )
