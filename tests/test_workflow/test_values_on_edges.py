...
class TestValuesOnEdges:
    @pytest.mark.asyncio
    async def test_loop_back_value_delivered_via_inputs(self) -> None:
        """``wf.loop``: the until-task returns ``(value, Next("continue"))``
        and the dep-less loop head observes that value as ``ctx.inputs`` on the
    ...
    @pytest.mark.asyncio
    async def test_branch_routed_value_delivered_via_inputs(self) -> None:
        """A branch's ``(dict, Next(label))`` reaches the dep-less routed target
        by name (the ``task(**upstream)`` shape) — distinct binding path from the
    ...
    @pytest.mark.asyncio
    async def test_declared_depends_on_wins_over_delivered_value(self) -> None:
        """A routed target WITH ``depends_on`` keeps the declared collection
    ...
    @pytest.mark.asyncio
    async def test_loop_back_merges_with_root_inputs(self, tmp_path) -> None:
        """A loop head that is also a workspace ROOT task sees the
        engine-injected ``{params, workdir}`` AND the loop-delivered keys merged
        in (delivered keys win) — same merge rule as SubWorkflow root_input
        forwarding."""
        seen_inputs: list[object] = []
        ...
        async def head(n: int | None = None, **rest: object) -> dict:
            ...
            # params (so rest IS params); from the second on, the loop-delivered
            # key blocks the unwrap, so the full {params, workdir} envelope arrives
            # in rest. Reconstruct the merged view the test asserts on either way.
            if "params" in rest:
