"""``PydanticAIRouter.stream_agentic`` — agentic-loop chunk translation.

Drives the router's emergent loop with pydantic-ai's offline ``TestModel``
plus one tool, and asserts the SDK-free
:data:`~molexp.agent.router.AgenticChunk` translation: text deltas, tool-call +
tool-result chunks, and a single terminal ``FinalChunk``.
"""
...
class TestStreamAgentic:
    async def test_dispatches_tool_and_ends_with_single_final_chunk(self) -> None:
        """TestModel calls the supplied tool, then the loop ends in one FinalChunk."""
        ...
        chunks = [
            chunk
            async for chunk in router.stream_agentic(prompt="inspect the project", tools=(peek,))
        ]
    ...
    async def test_streams_text_and_final_chunk_without_tools(self) -> None:
