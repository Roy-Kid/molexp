...
``test_workspace_layout.py``). This module keeps a small set of
...
def test_plan_mode_carries_config(tmp_path: Path) -> None:
    handle = PlanWorkspaceHandle.materialize(Workspace(tmp_path / "ws"), plan_id="cfg")
    mode = PlanMode(workspace_handle=handle, max_iterations=5)
...
def test_plan_mode_requires_workspace_handle() -> None:
    """``workspace_handle`` is required — ``PlanMode()`` with no args fails."""
