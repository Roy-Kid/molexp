"""Unit tests for ``molexp.agent.mcp.defaults`` — molmcp seeding (spec ``molmcp-agent-default``)."""
...
class TestSeedUserDefaults:
    """``seed_user_defaults`` — first-run molmcp seeding into the User config."""

    @pytest.mark.unit
    def test_writes_fresh_user_config_and_sentinel(self, tmp_path):
        """ac-005 — seeding a fresh User dir writes ``mcp.json`` + the sentinel."""
    ...
    def test_second_call_is_noop(self, tmp_path):
        """ac-006 — once seeded, a second call writes nothing (mtime unchanged)."""
        user_dir = tmp_path / "home" / ".molexp"
        config = user_dir / MCP_CONFIG_FILENAME
        sentinel = user_dir / MCP_SEEDED_FILENAME
    ...
    @pytest.mark.unit
    def test_honors_command_env_override(self, tmp_path, monkeypatch):
        """ac-007 — ``MOLEXP_MOLMCP_COMMAND="echo hi"`` rewrites command/args at seed time."""
    ...
    @pytest.mark.unit
    def test_respects_user_deletion(self, tmp_path):
        """ac-008 — a sentinel listing molmcp + molmcp absent from config → no re-seed."""
        user_dir = tmp_path / "home" / ".molexp"
    ...
    @pytest.mark.unit
    def test_store_construction_triggers_seed(self, tmp_path, monkeypatch):
        """ac-009 — constructing ``McpStore`` seeds molmcp and surfaces it via ``list()``."""
        ...
        by_name = {row.name: row for row in store.list()}
    ...
    def test_read_only_home_warns_and_returns_false(self, tmp_path, monkeypatch):
        """ac-010 — a read-only User dir warns rather than raising; returns ``False``."""
