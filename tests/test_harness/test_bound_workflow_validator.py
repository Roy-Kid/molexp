...
def test_baseline_pair_is_clean(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow

    ir, bw = _baseline()
    report = validate_bound_workflow(bw, ir, workspace_root=tmp_path)
...
def test_validate_bound_workflow_signature_and_import(tmp_path: Path) -> None:
    from molexp.harness import validate_bound_workflow as top
    from molexp.harness.schemas.validation import ValidationReport
    from molexp.harness.validators import validate_bound_workflow as via_pkg
    from molexp.harness.validators.bound_workflow import validate_bound_workflow as via_mod

    assert top is via_pkg is via_mod

    ir, bw = _baseline()
    report = top(bw, ir, workspace_root=tmp_path)
    assert isinstance(report, ValidationReport)


def test_phase_4_placeholder_comment_present() -> None:
...
def test_unknown_ir_task(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_duplicate_ir_task_binding(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_input_key_mismatch(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_output_key_mismatch(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_allowed_path_outside_workspace_absolute(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_allowed_path_outside_workspace_relative_escapes(tmp_path: Path) -> None:
...
def test_allowed_path_inside_workspace_is_clean(tmp_path: Path) -> None:
...
def test_missing_baseline_deny_root(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_missing_baseline_deny_ssh(tmp_path: Path) -> None:
    from molexp.harness.validators.bound_workflow import validate_bound_workflow
...
def test_edge_topology_mismatch(tmp_path: Path) -> None:
    ...
    bw = bw.model_copy(update={"edges": [DependencyEdge(source_task_id="b2", target_task_id="b1")]})
    report = validate_bound_workflow(bw, ir, workspace_root=tmp_path)
    assert "edge_topology_mismatch" in _codes(report)


def test_edge_topology_match_with_id_translation(tmp_path: Path) -> None:
...
def test_no_violations_when_clean(tmp_path: Path) -> None:
