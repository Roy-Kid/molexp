"""Tests for AssembleKnowledgeContext (vision-loop-05, spec RED set).

Locks:
- name == "assemble_knowledge_context"; artifact kind == "knowledge_context"
- Active KnowledgeItems get bodies, kind-priority ordered (FailureAnalysis
  before Finding); stale items excluded; Notes title-only
- Caps surface omission counts; an empty workspace still persists a digest
- PlanMode keeps 9 visible steps with the stage inside "Draft proposal";
  the proposal writer's AgentCallSpec carries the knowledge_context id
"""
...
def test_active_items_ordered_stale_excluded_notes_title_only(seeded, tmp_path: Path) -> None:
...
def test_empty_workspace_still_persists(seeded, tmp_path: Path) -> None:
    ws, _ = seeded
    digest = _digest(_ctx(Path(ws.resolve()), tmp_path / "run"))
    assert "no prior knowledge recorded" in digest


def test_run_dir_inside_workspace_finds_the_root(seeded, tmp_path: Path) -> None:
...
def test_outside_any_workspace_is_honest(tmp_path: Path) -> None:
    digest = _digest(_ctx(tmp_path / "bare", tmp_path / "run"))
    assert "not mounted inside a workspace" in digest


def test_caps_surface_omission_count(seeded, tmp_path: Path) -> None:
    ws, exp = seeded
...
def test_plan_mode_nine_steps_with_stage_in_draft_proposal() -> None:
...
def test_report_writer_receives_knowledge_context_id(seeded, tmp_path: Path) -> None:
