...
def test_record_llm_call_writes_cache_and_event(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
...
def test_ensure_plan_session_started_is_idempotent(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
...
def test_observer_records_gateway_trace(tmp_path: Path) -> None:
...
def test_prune_drops_old_and_over_count(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
    task = "plan-prune"
    ids: list[str] = []
    ...
    # Newest two of the original five (after age drop of first two, count-cap
    # may drop more — at most 2 files remain).
    ...
    event_ids = {e["payload"]["cache_id"] for e in llm_events}
    assert event_ids == set(remaining)


def test_prune_enforces_workspace_byte_budget(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
    big = "x" * 2000
    for i in range(4):
    ...
    total = 0
    for path in (root / "agent" / "_tasks").rglob("llm_cache/*.json"):
