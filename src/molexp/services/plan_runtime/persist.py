...
#: Runs in a child interpreter: argv[1] = dir holding the materialized package,
#: argv[2] = the importable entry module. Prints the display IR as JSON.
_PACKAGE_IR_SCRIPT = textwrap.dedent(
    ...
    compiled = module.build_workflow().compile()
...
def _compile_package_to_ir(ws: WorkflowSource) -> dict[str, JSONValue] | None:
    """Multi-file mode: build the display IR from ``ws.files`` in a subprocess.

    The harness never execs a multi-module generated program in-process
    (``ValidateWorkflowSource`` pre-checks files statically; the real build
    runs in ``CompileWorkflow``'s subprocess) — the display path honors the
    same trust posture. The files are materialized from the artifact (the
    authoritative copy — never the run's mutable ``generated/`` leftovers)
    into a private temp dir, the entry module is imported there by a child
    interpreter, and its IR comes back as JSON. Any failure is logged and
    yields ``None`` — this is observability sugar, never load-bearing.
    """
    try:
...
def persist_plan_workflow_to_experiment(run: Run, experiment: Experiment) -> bool:
    ...
    ir = _compile_package_to_ir(ws) if ws.files else _compile_source_to_ir(ws.source)
    if ir is None:
        return False
