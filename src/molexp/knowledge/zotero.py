"""Read-only Zotero ingestion for ``molexp.knowledge`` (okf-07-02).

Reads a local ``zotero.sqlite`` (opened read-only) and yields :class:`ZoteroItem`
records. PDFs are *resolved to a path under Zotero's own ``storage/`` tree* —
no bytes are read or copied (mirrors ``molexp.workspace.library.zotero``).
``Library.import_zotero`` turns these into ``Reference`` Concepts.
"""
