...
Critical design note: molexp task bodies **write files under
``ctx.workdir`` and publish them with ``ctx.register_artifact`` /
``ctx.register_metric``** — that is the product surface, not a purity
violation. Reviewers that flag workdir I/O as "side effects" block every
correct plan. Same for high-k harmonic bonds as a testable stand-in for
rigid geometry in offline dry-runs.
"""
...
SYSTEM_PROMPT = (
    ...
    "- Write files under `ctx.workdir` and publish them via "
    "`ctx.register_artifact(path, mime=...)` — this is how the UI and audit "
    "trail see products. That is NOT a forbidden side effect.\n"
    "- Return scalars via `ctx.register_metric(key, value)`.\n"
