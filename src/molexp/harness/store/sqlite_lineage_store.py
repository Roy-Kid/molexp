"""SQLite implementation of :class:`ArtifactLineageStore`.

Scoped strictly to **pipeline-artifact lineage**: ``derived_from`` edges
between harness artifact ids, each stamped (when known) with the producing
``stage`` name and the pipeline ``run_id``. Run-level provenance (params,
config, environment, workflow identity) is owned by ``molexp.workspace``.
...
class SQLiteArtifactLineageStore:
    ...
    def add_edge(
        ...
        *,
        ...
        """Record one lineage edge, stamped with its pipeline context.

        ``stage`` is the name of the pipeline stage that derived ``child_id``;
        ``run_id`` is the run the pipeline executed under. Idempotent on the
        ``(parent_id, child_id, relation)`` primary key; a duplicate write
        backfills a missing ``stage`` / ``run_id`` (mirroring
        ``FileArtifactStore``'s parent-id merging) but never overwrites what
        the first writer recorded.
        """
        with self._lock:
            self._conn.execute(
                "INSERT INTO artifact_edges "
                "(parent_id, child_id, relation, created_at, stage, run_id) "
                "VALUES (?, ?, ?, ?, ?, ?) "
                "ON CONFLICT (parent_id, child_id, relation) DO UPDATE SET "
                "stage = COALESCE(artifact_edges.stage, excluded.stage), "
                "run_id = COALESCE(artifact_edges.run_id, excluded.run_id)",
                (
                    parent_id,
                    child_id,
                    relation,
    ...
    def lineage_graph(self, artifact_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            rows = self._conn.execute(
                f"SELECT parent_id, child_id, relation, stage, run_id FROM artifact_edges "
                f"WHERE parent_id IN ({placeholders}) OR child_id IN ({placeholders})",
                (*ids, *ids),
            ).fetchall()
        edges = [
            {"parent_id": p, "child_id": c, "relation": r, "stage": s, "run_id": rid}
            for p, c, r, s, rid in rows
