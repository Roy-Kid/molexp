...
structured :class:`PlanWorkflowIR` — the flow + topology. Every LLM-backed
...
from molexp.harness.schemas import AgentCallSpec, PlanArtifactRef, PlanWorkflowIR
...
class ExtractWorkflowIR(Stage):
    """Expand a concrete experiment_spec into a structured PlanWorkflowIR via gateway."""
    ...
    async def run(self, ctx: HarnessRunContext) -> PlanArtifactRef:
        ...
        call = AgentCallSpec(
            ...
            output_schema=PlanWorkflowIR.model_json_schema(),
        )
