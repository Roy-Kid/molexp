...
class DiskTaskBoard:
    ...
    def with_task_updated(self, task_id: str, changes: dict[str, object]) -> DiskTaskBoard:
        ...
        if task is None:
            raise TaskNotFoundError(f"task {task_id!r} not found on board")
        allowed = {"name", "acceptance", "status"}
        update: dict[str, Any] = {}
        ...
        * ``remove`` — list of task ids to drop
        ...
        if isinstance(updates, dict):
            for tid, changes in updates.items():
                ...
                update: dict[str, Any] = {}
                if "name" in changes:
                    update["name"] = str(changes["name"])
                if "acceptance" in changes:
                    from molexp.harness.plan.task_board import coerce_acceptance

                    update["acceptance"] = coerce_acceptance(changes["acceptance"])
                if "status" in changes:
                    update["status"] = TaskStatus(str(changes["status"]))
                board = place_task(board, task.model_copy(update=update))
