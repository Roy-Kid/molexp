...
:class:`~molexp.agent.router.Router`, the
:class:`~molexp.agent.execution_env.ExecutionEnv` subprocess sandbox,
and the :class:`~molexp.agent.hooks.HookRegistry`. ``AgentRunner``
builds the bundle once per :meth:`run` and passes it through to the
mode; the mode forwards events to its
...
class AgentRuntime:
    ...
    Plain frozen dataclass — no methods. Modes interact with the four
