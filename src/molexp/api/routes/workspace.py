...
from ..dependencies import get_workspace, get_workspace_folder_store, WorkspaceFolderStore
...
@router.post("/workspace/folders", response_model=WorkspaceFolderResponse, status_code=201)
...
@router.get("/workspace/folders/{folder_id}/browse", response_model=FolderBrowseResponse)
...
def _resolve_file_path(folder_id: str, path: str, workspace, store: WorkspaceFolderStore) -> tuple[Path, Path]:
...
def classify_folder(path: str, workspace=Depends(get_workspace)) -> EntityClassificationResponse:
