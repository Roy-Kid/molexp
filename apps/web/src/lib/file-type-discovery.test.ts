  ...
  it("recursively collects only file nodes", () => {
    const nodes = [
      ...
      },
      {
        ...
        relPath: "metrics.mlp.jsonl",
        type: "file",
        size: 200,
      },
    ];
    const flat = flattenFileNodes(nodes);
    expect(flat.map((f) => f.relPath).sort()).toEqual(["logs/log.lammps", "metrics.mlp.jsonl"]);
  });
...
describe("discoverPluginsForObject", () => {
  it("returns matched contributions with their files", () => {
    const contribution: FileTypeContribution = {
      ...
      value: "metrics",
      label: "Metrics",
      matcher: { patterns: ["metrics.mlp.jsonl"] },
      Component: noopComponent,
    };
    registerFileTypeContribution(contribution);

    const found = discoverPluginsForObject("run", [ctx("metrics.mlp.jsonl"), ctx("other.txt")]);
  ...
  it("excludes contributions with zero matched files", () => {
    ...
    const found = discoverPluginsForObject("run", [ctx("metrics.mlp.jsonl")]);
