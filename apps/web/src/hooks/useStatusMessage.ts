...
export function useStatusMessage(): {
  ...
  const applyStatus = useCallback(
    (nextText: string, nextType: StatusReportType, nextProgress?: number) => {
      ...
      // Active progress (including 0 = "started, unknown fraction") stays until
      // replaced by the producer (status strip / remote index). Warnings/errors
      // persist until dismissed or replaced.
      //
      // Note: producers should prefer `progress=undefined` for pure info tips
      // and only pass a number while a bar should stay visible.
