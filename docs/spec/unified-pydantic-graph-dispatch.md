...
**Current profile convention**: `jobs: <int>` is user data (schemaless — see §2 and `docs/spec/molcfg-profiles.md`). The CLI `-j` flag takes precedence over the profile; profile values > 1 enable parallelism; default is 1 (backwards compatible).
...
6. **Old CLI flags are kept as aliases.** `--local` / `--slurm` / `--pbs` / `--lsf` map onto the new `--backend` + profile combo; no forced deprecation window.
...
- Related spec: `docs/spec/molcfg-profiles.md` (profile mechanism)
