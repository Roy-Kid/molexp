...
| `molexp.agent`      | Optional LLM layer — `AgentRunner` (`mode="text"` one-shot, `mode="agentic"` one ReAct per turn) with persisted `AgentSession`s, built on PydanticAI (lazy-loaded) |
| `molexp.harness`    | Experiment orchestrator — audited stages over a content-addressed Run (artifact lineage, approval gates, executors). Two bundles: `run_plan` (interactive planning behind a hard review gate, then deterministic realization into a compiled workflow) and `ChatMode` (exploratory, scratch-only). The production `molexp plan` entry point |
