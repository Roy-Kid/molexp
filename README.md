...
| `molexp.harness`    | Experiment orchestrator — audited stages over a content-addressed Run (artifact lineage, approval gates, executors). Two bundles: `Plan` (interactive planning behind a hard review gate, then deterministic realization into a compiled workflow) and `ChatMode` (exploratory, scratch-only). The production `molexp plan` entry point |
