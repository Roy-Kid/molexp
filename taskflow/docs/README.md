...
Taskflow is organized around a strict separation of concerns:
...
```python
from taskflow.task_base import Task, EmptyConfig
...
Use `taskflow.compiler.compile_graph` to obtain a `CompiledGraph`. The compiler performs DFS
...
See `src/taskflow/examples/` for full runnable samples.
