"""``AgentRunner`` + ``ChatLoop`` — multi-turn conversation with named sessions.

Demonstrates the public agent surface after the harness restructure:
``AgentRunner``, ``ChatLoop``, the runtime ``AgentSession`` (the harness
``Session`` entry-tree), and the ``AgentRunResult`` ``runner.run``
returns.

``ChatLoop`` is the minimal :class:`~molexp.agent.loop.AgentLoop`: each
turn is one user input -> one LLM round-trip -> one
:class:`~molexp.agent.loop.AgentRunResult`. Every loop receives an
:class:`~molexp.agent.runtime.AgentRuntime` (the frozen session + router
+ execution-env + hooks bundle) that ``AgentRunner`` builds and injects;
``run`` drains the loop's ``AgentEvent`` stream and folds it into the
terminal result.
...
def _make_runner(workspace: Path) -> AgentRunner:
    """Build a ChatLoop runner backed by an on-disk workspace.
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # loop rebuilds prior context from the entry-tree.
